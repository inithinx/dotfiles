{config, lib, pkgs, microvm, ...}:{
  ...
  environment.systemPackages = with pkgs; [ kubectl helm iptables ethtool nfs-utils neovim htop-vim ];
  ...
  boot.kernelModules = [ "br_netfilter" "rbd" "overlay" ];
  ...
  services.k3s = {
    ...
    serverAddr = if config.networking.hostName != "alpha"
      then "https://10.0.0.11:6443" 
    else ""; 
    # Conditional role assignment
    role = if config.networking.hostName == "alpha" 
      then "server" 
    else "agent";
